...
private:
    ...
    XMFLOAT3 mBaseSpotLightDirections = { 0.0f, -1.0f, 1.0f };
    POINT mLastMousePos;
};
    ...
    : D3DApp(hInstance)
{
    ...
    // 初始化聚光灯参数
    mSpotLight.Strength = { 0.9f, 0.9f, 0.2f };    // 光照强度
    mSpotLight.Position = { 0.0f, 15.0f, 0.0f }; // 聚光灯位置（场景上方）
    mSpotLight.Direction = { 0.0f, -1.0f, 1.0f };  // 初始照射方向（斜向下）
    mSpotLight.FalloffStart =0.1f;                // 衰减开始距离
    mSpotLight.FalloffEnd = 33.0f;                 // 衰减结束距离（最大照射范围）
    mSpotLight.SpotPower = 44.0f;                  // 聚光圆锥硬度（值越大，边缘越锐利）
...
void ShadowMapApp::Update(const GameTimer& gt)
{
    ...
    mLightRotationAngle += sin(0.1f*gt.DeltaTime());

    XMMATRIX R = XMMatrixRotationY(mLightRotationAngle);
    XMVECTOR lightDir = XMLoadFloat3(&mBaseSpotLightDirections);
...
void ShadowMapApp::UpdateShadowTransform(const GameTimer& gt)
{
    ...
    XMVECTOR lightDir = XMLoadFloat3(&mSpotLight.Direction);
    ...
    //改成透视投影：
     lightProj = XMMatrixPerspectiveOffCenterLH(l, r, b, t, n, f);
    //
...
void ShadowMapApp::UpdateMainPassCB(const GameTimer& gt)
{
    ...
    mMainPassCB.Lights[0] = mSpotLight; // 直接赋值Light结构体
    /*mMainPassCB.Lights[1].Direction = mRotatedLightDirections[0];
